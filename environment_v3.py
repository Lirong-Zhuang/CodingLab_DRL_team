...
class Environment_v3(object):
    ...
    # Using CNN to realize the fixed input size. Dimension of obs is 5*5*5
    def get_obs(self):
        
        #  obs initial
        obs = np.zeros((5, self.vertical_cell_count, self.horizontal_cell_count), dtype=np.float32)
        ...
        # Channel 3: target location
        obs[3, self.target_loc[0], self.target_loc[1]] = 1.0

        # Channel 4: item location + remaining item time
        for item_loc, item_time in zip(self.item_locs, self.item_times):
            ...
            obs[4, item_loc[0], item_loc[1]] = time_left
