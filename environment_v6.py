...
class Environment_v6(object):
    ...
    # Using CNN + MLP to realize the fixed input size. Dimension of obs is 3*5*5 + 2
    def get_obs(self):
        ...
        for item_loc, item_time in zip(self.item_locs, self.item_times):
            ...
            grid_obs[1, r, c] = 1.0
            grid_obs[2, r, c] = time_left

        # MLP input: 2 scalar features
        scalar_obs = np.array([
            self.agent_load / self.agent_capacity,
            (self.episode_steps - self.step_count) / self.episode_steps
        ], dtype=np.float32)
