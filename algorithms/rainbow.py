...
class RainbowDQN:
    def __init__(self, env):
        ...
        self.file_name = f'RainbowDQN.'
        self.network_name = 'Rainbow DQN Network (release implementation)'
...
class EncoderDecoderRainbowDQN:
    def __init__(self, env, encoder_path=None, freeze_encoder=False, encoder_type="cnn"):
        ...
        self.variant = self.env.variant
        self.data_dir = self.env.data_dir
        self.file_name = f'EncoderDecoderRainbowDQN.'
        self.network_name = 'Rainbow DQN Network (separate encoder/Q-head implementation)'
        self.training_step = 0
        ...
        if isinstance(checkpoint, dict) and "encoder_state_dict" in checkpoint:
            ...
            if checkpoint_encoder_type is not None and checkpoint_encoder_type != self.encoder_type:
                raise ValueError(
                    ...
                    f"but EncoderDecoderRainbowDQN was built with {self.encoder_type!r}."
                )
            if checkpoint_feature_dim is not None and checkpoint_feature_dim != self.feature_dim:
                raise ValueError(
                    ...
                    f"but EncoderDecoderRainbowDQN expects {self.feature_dim}."
                )
